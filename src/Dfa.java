...
public class Dfa {
     ...
     */
    private HashSet<String> Σ = new HashSet<>();
     ...
     */
    private Hashtable<String, String> δ = new Hashtable<>();
        ...
        try {
            ...
            while (scan.hasNextLine()){
                ...
                switch (count){
                    ...
                    case 2:
                        line_split = line.split("\\s+");
                        Collections.addAll(Σ, line_split);
                        break;
                    case 3:
                        s = line;
                        break;
            ...
            if (line.equals("")){
                ...
                if (trace) {
                    for (int x = 1; x < split_line.length; x++) {
                        ...
                        if (dfa1.δ.containsKey(input_string)) {
                            current_state = dfa1.δ.get(input_string);
    ...
    public void print_Dfa(){
        ...
        output.add(line);
        line = "Sigma = " + line_builder(Σ);
        output.add(line);
        line = "q_0 = " + s;
        output.add(line);
        line = "F = " + line_builder(F);
        output.add(line);
        line = state_builder(δ);
        output.add(line);
        for (String s : output)
    ...
    /**
     * Function that takes a HashSet and returns its String representation
     *
     * @param set HashSet to be printed
     * @return String representation of set
     */

    public String line_builder(HashSet<String> set){
        String line = "{";
        int count = 0;
        int size = set.size();
        for (String s: set){
    ...
    /**
     * Function that takes a Hashtable and returns its String representation
     *
     * @param table Hashtable to be printed
     * @return String representation of table
     */

    public String state_builder(Hashtable<String, String> table){
        String line = "";
        int count = 0;
        int size = table.size();
